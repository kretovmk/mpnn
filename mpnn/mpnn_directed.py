...
class MPNNdirected:
    ...
    def make_opt_step_batched(self, smiles, batch_x, batch_x2, batch_y, t):
        self.opt.zero_grad()
        loss = Variable(torch.zeros(1, 1))
        fold = Fold()
        ...
        for i in range(len(batch_x)):
            print(i)
            smile = smiles[i]
            g_dict[smile], h_dict[smile] = batch_x[i]
            _, h2_dict[smile] = batch_x2[i]
